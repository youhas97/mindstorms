...
school_computer = False
try:
    temp_location = os.getcwd()
    os.chdir("C:/Python27")
    school_computer = True
except OSError:
    pass
import socket, select, logging, time, threading
import numpy as np
import time
...
class Poller(threading.Thread):
    def __init__(self, receiver_instance):
        self.__receiver = receiver_instance
        threading.Thread.__init__(self)

    def run(self):
        while True:
            time.sleep(0.1)
            self.__receiver.lock.acquire()
            self.__receiver.frame = self.__receiver.get_frame_internal()
            self.__receiver.updated = True
            self.__receiver.lock.release()
...
class Receiver():
    ...
    def get_frame_internal(self):
        self.send_command("get_frame=")
        return self.receive_data()

    def get_frame(self):
        self.lock.acquire()
        temp_bool = self.updated
        self.lock.release()
        while (not temp_bool) and (self.converted_frame == None):
            self.lock.acquire()
            temp_bool = self.updated
            self.lock.release()
            if not temp_bool:
        ...
        if self.updated:
            self.updated = False
            self.converted_frame = self.__convert[self.__mode](self.frame)
        self.lock.release()
